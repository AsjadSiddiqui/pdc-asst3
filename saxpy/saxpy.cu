...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    cudaMalloc(device_x_ptr, vector_size);
    cudaMalloc(device_y_ptr, vector_size);
    cudaMalloc(device_result_ptr, vector_size);
    ...
    cudaMemcpy(device_x, xarray, vector_size, cudaMemcpyHostToDevice);
    cudaMemcpy(device_y, yarray, vector_size, cudaMemcpyHostToDevice);

    // run CUDA kernel. (notice the <<< >>> brackets indicating a CUDA
    // kernel launch) Execution on the GPU occurs here.
    double kernelStartTime = CycleTimer::currentSeconds();
    saxpy_kernel<<<blocks, threadsPerBlock>>>(N, alpha, device_x, device_y, device_result);
    // make sure the kernel is finished before going forward
    ...
    cudaMemcpy(resultarray, device_result, vector_size, cudaMemcpyDeviceToHost);
    ...
    float kernelBandwidth = GBPerSec(totalBytes, kernelTimeInSeconds);
